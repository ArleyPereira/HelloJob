...
class SetupProfileViewModel : ViewModel() {
    ...
    private var _state = MutableStateFlow(SetupProfileState())
    var state: StateFlow<SetupProfileState> = _state

    init {
        getProfile()
    }

    fun dispatchAction(action: SetupProfileAction) {
        when (action) {
            is SetupProfileAction.Update -> {
                update()
            }

            is SetupProfileAction.OnTextFieldChanged -> {
                onTextFieldChanged(value = action.value, type = action.type)
            }

            is SetupProfileAction.SetBackResult -> {
                setBackResult(parameter = action.parameter)
            }

            is SetupProfileAction.OnChangeImage -> {
    ...
    private fun setBackResult(parameter: SetupProfileParameter) {
        viewModelScope.launch {
            parameter.genre?.let {
                _state.update { currentState ->
